...
class ApproachChargingStationService(BaseApproachClass):
    ...
    def search_for_base_station(self):
        ...
        if "small_hauler" in self.robot_name:
            rospy.sleep(0.8)
            self.check_for_base_station(self.boxes.boxes)
            if self.check_for_charging_station_features(self.boxes.boxes)>=2:
            ...
            if self.rover_boxes:
                for object_ in self.rover_boxes:
                    ...
                    if dist < 6.0:
                        rospy.sleep(10)
                        self.stop()
                        print("Robot in front - distance {} - Sleep for ten seconds".format(dist))
    ...
    def check_for_charging_station_features(self, boxes):
        number_features = 0
        for box in boxes:
            if box.id == 1 or box.id ==7 or box.id ==10 or box.id ==14 or box.id ==15: # bin id == 6 -- regolith id == 12
                number_features+=1
